...
G3는 위 caveat 6종을 **한 번만 서명한 root delegation**으로 재사용해 20회차 × 500 USDC를
연속 전송한다. 회차 수와 금액은 §"파생: G3에서 필요한 회차 수"에서 이미 정한 값 그대로다.

| 항목 | 값 |
| --- | --- |
| 회차 수 | 20 |
| 회차당 전송액 | `500_000_000` (500 USDC) |
| 회차 n(1..20)의 블록 타임스탬프 | `1786068491 + 21600 * n` |
| 첫 회차가 n=1인 이유 | `TimestampEnforcer`가 strict `>` 비교라 n=0(기준 ts 그대로)이면 `early-delegation` |

### 실측 period 분포 — 3, 4, 4, 4, 4, 1 (정직하게 기록)

period index(0-based) = `floor(21600 * n / 86400)` = `floor(n / 4)`. n=1..20을 이 식에
대입하면 period 0~5에 다음과 같이 분포한다.

| period index | 포함 회차 n | 회차 수 | 합계 (USDC) | 일일 한도 대비 |
| --- | --- | --- | --- | --- |
| 0 | 1, 2, 3 | 3 | 1,500 | ≤ 2,000 |
| 1 | 4, 5, 6, 7 | 4 | 2,000 | = 2,000 (정확히 소진) |
| 2 | 8, 9, 10, 11 | 4 | 2,000 | = 2,000 |
| 3 | 12, 13, 14, 15 | 4 | 2,000 | = 2,000 |
| 4 | 16, 17, 18, 19 | 4 | 2,000 | = 2,000 |
| 5 | 20 | 1 | 500 | ≤ 2,000 |
| **합계** | | **20** | **10,000** | |

전 period가 일일 한도 2,000 USDC 이하다(1~4번 period는 정확히 한도까지, 0·5번은 그
아래). **이 분포를 4,4,4,4,4로 맞추려고 `BASE_TIMESTAMP`/`STEP_TIMESTAMP_OFFSET`/
`erc20Period.startDate`/회차 수를 옮기지 않았다.** 회차 간 오프셋 21600초(6시간)와
strict `TimestampEnforcer`가 만들어내는 그대로의 결과를 기록한다.

### 정정 노트 — "5일 × 4회" 근사였다

위 §"파생: G3에서 필요한 회차 수"의 "500 USDC × 일 4회 = 일 2,000 USDC... 5일 × 4회 =
20회"라는 서술은 회차 간 오프셋(21600초)을 고려하지 않은 **근사**였다. 실제로는 6개의
period 창에 3,4,4,4,4,1로 분포한다(오프셋이 기준 시각에서 시작하지 않고 `+21600`부터
시작하기 때문에 첫 창과 마지막 창이 온전히 4회씩 차지 않는다). **회차 수 20과 총액
10,000 USDC는 그대로다** — 근사가 틀린 것은 "몇 개의 창에 걸치는가"였지 "몇 회가
필요한가"가 아니다.

### 오라클 환산 vs 명목 페그 — 정정 노트

이 문서 상단 "시작 포트폴리오" 표의 USDC 행 "$10,000.00"은 **명목 페그 근사**다(1
USDC = $1.00으로 취급). G3는 실제 Chainlink USDC/USD 피드로 환산하며, 포크 블록에서
`answer = 99_976_752`(8 decimals) = **$0.99976752/USDC**다. 오라클 환산 시작 USDC 가치는
$10,000.00이 아니라 **$9,997.6752**이고, 시작 포트폴리오 합계도 표의 $28,981.11과
정확히 일치하지 않는다(ETH 환산분 $18,981.11은 동일, USDC 환산분만 오라클값으로
바뀐다). 수량(USDC 10,000 / ETH 10)은 표 그대로 바꾸지 않는다 — 바뀌는 것은 USD
환산값뿐이다.

### 오라클 신선도 상한 86400초 — G3 구현 시 고정한 값

`ORACLE_MAX_STALENESS_SECONDS = 86_400`(하루)은 G3 구현 시 코드와 독립 검증기에 고정했다.
다만 USDC/USD 피드 age(포크 블록 대비 65,268초)를 관측하기 전에 이 상한을 선택했다는
역사적 기록은 없다. 따라서 사전선정 또는 관측값과 무관한 선택이었다고 주장하지 않는다.
