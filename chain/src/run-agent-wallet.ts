...
async function main(): Promise<void> {
  ...
  const bundle = parseAgentWalletExecutionBundle(raw);
  const result = await executeAgentWalletBundle({
    bundle,
    rpc: new ViemAgentWalletRpc(rpcUrl),
    broadcast: args.broadcast,
    send: createAgentWalletCliSender(),
  });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
}

main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "unknown Agent Wallet runtime failure";
  process.stderr.write(`Agent Wallet execution failed closed: ${message}\n`);
  process.exitCode = 1;
});
